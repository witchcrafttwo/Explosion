export class UIController {
  ...
  showResult(outcome) {
    ...
    const heading = outcome.result === "win" ? "勝利！" : outcome.result === "lose" ? "敗北..." : "引き分け";
    this.resultText.textContent = heading;
  ...
  updateInkMeter() {
    ...
    const localEntry = stats.find((entry) => entry.playerId === this.state.playerId);
    const opponentEntry = stats.find((entry) => entry.playerId && entry.playerId !== this.state.playerId);
    const clampPercent = (value) => Math.max(0, Math.min(100, Math.round((value || 0) * 100)));
    const localPercent = clampPercent(localEntry?.coverage || 0);
    const opponentPercent = clampPercent(opponentEntry?.coverage || 0);
    this.inkMeterSelf.style.width = `${localPercent}%`;
    this.inkMeterOpponent.style.width = `${opponentPercent}%`;
    if (this.inkMeterSelfLabel) {
      this.inkMeterSelfLabel.textContent = `${localPercent}%`;
    }
    if (this.inkMeterOpponentLabel) {
      this.inkMeterOpponentLabel.textContent = `${opponentPercent}%`;
    }
  }
  ...
  updateAbilityMeters() {
    this.updateAbilityMeter(
      this.grenadeMeter,
      this.grenadeTimer,
      this.state.cooldowns.grenade,
      this.state.cooldownMax.grenade
    );
    this.updateAbilityMeter(
      this.homingMeter,
      this.homingTimer,
      this.state.cooldowns.homing,
      this.state.cooldownMax.homing
    );
  }
  ...
  renderResultDetails(scores) {
    ...
    const local = scores.find((entry) => entry.playerId === this.state.playerId);
    const opponent = scores.find((entry) => entry.playerId && entry.playerId !== this.state.playerId);
    const blocks = [];
    if (local) {
      blocks.push(this.buildResultGroup(local, "あなた", "self"));
    }
    if (opponent) {
      blocks.push(this.buildResultGroup(opponent, "相手", "opponent"));
    }
